...
path_shortcode = ''
shortcode_set = set()
end_cursor = 'QVFEX29xVm1iVGVha0oxdWdaSU1LTjJsVlBNY1NOSzNKTTZPbDVWT2JEZ09fWkxmNVRwX0ZBaEEtc2dTNDhGbmdVZzIwSHRUY1A3ODAwRU1zZGJxM0lXbQ=='
has_next_page = True
each_article_url = ''
    ...
    try:
        ...
        for item in items:
            if item.text().strip().startswith('window.__additionalDataLoaded'):
                ...
                #
                # tag_data_str = ''
                # for each_tag in content :
                #     tag_data_str += each_tag.split('\n')[0]
                #print(tag_data_str)

                with open(path_dir +'\Tag.txt' , 'a', encoding='utf8') as f:
                    f.write(content)
